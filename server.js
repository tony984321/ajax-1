...
var server = http.createServer(function(request, response){
  ...
  if(path === '/index.html'){
    ...
    const result = array.map(a => `<li>${a.id}</li>`).join('')
    string = string.replace('{{ page1 }}', `<ul id="container">${result}</ul>`);
    ...
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/html;charset=utf-8')
    response.write(fs.readFileSync('src/1.html'))
    response.end()
  }else if(path === '/3.xml'){
    response.statusCode = 200
    response.setHeader('Content-Type', 'text/xml;charset=utf-8')
    response.write(fs.readFileSync('src/3.xml'))
    response.end()
  }else if(path === '/5.json'){
    response.statusCode = 200
